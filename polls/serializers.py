...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    poll = serializers.SlugRelatedField(queryset=Poll.objects.all(),
                                        slug_field='id')
    question = serializers.SlugRelatedField(queryset=Question.objects.all(),
                                            slug_field='id')
    ...
    def validate(self, attrs):
        ...
        try:
            if question_type == "radiobutton" or question_type == "Text":
...
class ChoiceSerializer(serializers.Serializer):
    id = serializers.IntegerField(read_only=True)
    choice_question = serializers.SlugRelatedField(queryset=Question.objects.all(), slug_field='id')
    choice_text = serializers.CharField(max_length=200)

    def validate(self, attrs):
        try:
            obj = Choice.objects.get(question=attrs['question'].id,
...
class QuestionSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if question_type == 'radiobutton' or \
        question_type == 'checkbox' or \
        question_type == 'Text':
            return attrs
        raise serializers.ValidationError('Question type can be only'
                                          'radio checkbox, text')
...
class PollSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if 'pub_date' in validated_data:
            raise serializers.ValidationError({'date_start':
